...
export class PersonalInformationPage {
  ...
  constructor() {
    ...
    this.pageTitleLabel = element(by.tagName('h1'));
    this.uploadFileInput = element(by.css('[name="photo"]'));
  }
  ...
  private continentOption(name: string): ElementFinder {
    return element(by.css('[name="continents"]')).element(by.cssContainingText('option', name));
  }

  private seleniumCommandOption(name: string): ElementFinder {
    return element(by.css('[name="selenium_commands"]'))
